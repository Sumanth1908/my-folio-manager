...
export default function InvestmentDetails({ account, symbol }: InvestmentDetailsProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            >
                <form onSubmit={handleConfirmSell} className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Quantity to Sell</label>
                            <input
                                type="number"
                                step="any"
                            ...
                            />
                            <p className="text-[10px] text-muted-foreground mt-1 uppercase">Available: {sellingHolding?.quantity}</p>
                        </div>
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Sell Price ({account.currency})</label>
                            <input
                                type="number"
                                step="0.01"
                                required
                    ...
                    {sellQuantity && sellPrice && (
                        <div className="p-4 rounded-xl border border-destructive/10 bg-destructive/5 text-destructive text-xs font-bold flex justify-between items-center">
                            ...
                            <span className="text-base font-black tabular-nums">
                                {symbol}{(parseFloat(sellQuantity) * parseFloat(sellPrice)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </span>
                        </div>
                    )}
